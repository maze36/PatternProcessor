...
public class ConcaveHull {
	...
	public ArrayList<Point> calculateConcaveHull(ArrayList<Point> pointArrayList, Integer k) {
		...
		while ((currentPoint != firstPoint || step == 2) && pointArraySet.size() > 0) {
			...
			// after 3 steps add first point to dataset, otherwise hull cannot
			// be closed
			if (step == 5) {
			...
			// check if clockwise angle nearest neighbors are candidates for
			// concave hull
			Boolean its = true;
			int i = -1;
			while (its && i < clockwisePoints.size() - 1) {
				...
				// check if possible new concave hull point intersects with
				// others
				int j = 2;
				its = false;
